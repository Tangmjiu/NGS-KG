...
/// - Errors (file not found, network failure) → same placeholder
class LocalCoverArt extends StatelessWidget {
  final String? url;
  final double? size;
  final BoxFit fit;
  final double? borderRadius;
  final Uint8List? coverData; // raw cover bytes (fallback when file cache unavailable)
  ...
  Widget _buildContent(BuildContext context, double imageSize) {
    // Raw cover data (from metadata reader, no file cache needed)
    ...
    // Raw file system path (no scheme) → try as local file
  ...
  /// Returns true if [path] looks like a raw file system path
  ...
  bool _isRawFilePath(String path) {
    ...
    // Relative path with separator (e.g. `./cover.jpg`, `folder/file.jpg`)
    if (path.startsWith('./') || path.startsWith('.\\')) return true;
    // Contains path separator and has a file extension (heuristic)
