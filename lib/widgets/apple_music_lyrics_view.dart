...
class KaraokeWordWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // 正在吟唱中，对该字/音节使用 ShaderMask + LinearGradient 渐进式渲染
    ...
    return ShaderMask(
      ...
      shaderCallback: (bounds) {
        return LinearGradient(
          ...
          stops: [progress, progress], // 突变渐变点，实现无羽化的精准逐字卡拉OK高亮
...
/// 单行歌词容器 (负责卡拉OK/静态文字排版，以及活动行的变焦与半透明失焦动效)
class LyricLineContainer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (line.words != null && line.words!.isNotEmpty && isActive) {
      ...
      // 静态行渲染：已唱过使用稍亮的颜色，未唱到使用默认较暗淡的 textStyle 颜色
      ...
      builder: (context, constraints) {
        return Stack(
          ...
          children: [
            ...
            // ── 手动拖拽时：极简悬浮中线与 Seek 播放跳转指示器 (AMLL 精髓) ──
            if (_isUserDragging)
              Positioned(
                left: 16,
                right: 16,
